...
First, start with the smallest cog in this vast bureaucratic machine: the **Bureaucrat**. A Bureaucrat must have:
...
However, the intern has one key ability: the `makeForm()` function. This function takes two strings as parameters: the first one represents the name of a a form, and the second one represents the target of the form. It returns a pointer to a `AForm` object (corresponding to the form name passed as a parameter), with its target initialized to the second parameter.
...
You must avoid unreadable and messy solutions, such as using an excessive `if/elseif/else` structure. This kind of approach will not be accepted during the evaluation process. As usual, you must test everything to ensure it works as expected.
